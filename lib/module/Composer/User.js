...
module.exports = (function () {
  ...
  var logout = function logout(token, sessionId) {
    return redisClient.get('sess:' + sessionId)
      .then(function (result) {
        var resultJS = JSON.parse(result);
  ...
  var login = function login(userObj, sessionId) {
    ...
    return Model
      ...
      .then(function (findUser) {
        ...
        var checkPassword = bcrypt.compareSync(userPassword, findUser.get('password')); // true
        if (checkPassword) {
          return setTokenWithRedisSession({nick: findUser.get('nick'), id: findUser.get('id')}, sessionId);
        } else {
