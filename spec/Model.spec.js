    ...
    it('>> findOne <<', function (done) {
      Goblin('Composer', function (G) {
        ...
        model
          ...
          // Create Post
          .then(function(prefixs) {
            seed.prefixs = prefixs;
            return model
              .Post
              .create({
                title: '제목',
          ...
          .then(function (subcomment) {
            seed.subcomment = subcomment;
            return seed.user.addLikePost(seed.post);
          })

          .then(function (user) {
            return seed.user2.addLikePost(seed.post);
          })

          .then(function (user) {
            return seed.user.addLikePost(seed.post2);
          })

          .then(function (user) {
            return seed.user2.addLikePost(seed.post2);
          })

          .then(function (user) {
            return seed.post.setUserLikePost(seed.user, { through: 'delete' });
          })


          // Like Comment
          .then(function () {
            return seed.user.addLikeComment(seed.comment);
          })

          .then(function (user) {
            return seed.user2.addLikeComment(seed.comment);
          })

          .then(function (user) {
            return seed.comment.setUserLikeComment(seed.user, { through: 'delete' });
          })

          // Like SubComment
          .then(function () {
            return seed.user.addLikeSubComment(seed.subcomment);
          })

          .then(function (user) {
            return seed.user2.addLikeSubComment(seed.subcomment);
          })

          .then(function (user) {
            return seed.subcomment.setUserLikeSubComment(seed.user, { through: 'delete' });
          })
          ...
          .then(function() {
            return seed.pageview.getPageViewLogs({ include: [ model.User ] });
          })

          // Test
          .then(function (pageViewLogs) {
            console.dir(JSON.parse(JSON.stringify(pageViewLogs)), { depth: null });
            return model
              .Tag
              .findOne({ name: 'tag' })
          })
          ...
          .then(function (tag) {
            return seed.post.addTag(tag);
          })

          .then(function(taggedPost) {
            console.dir(JSON.parse(JSON.stringify(taggedPost)), { depth: null });
            return model
              .Post
              .findOne({
                include: [
          ...
          .then(function (post) {
            console.dir(JSON.parse(JSON.stringify(post)), { depth: null });
          ...
          .then(function (prefix) {
            console.dir(JSON.parse(JSON.stringify(prefix)), { depth: null });
            return model.Club.find({
              where: {id : 1},
              include: [
                model.ClubSetting,
                {
                  ...
                  include: [
                    { model: model.MembershipPermission, where: { permission_value: 1 }, include: [model.Membership, model.Permission] },
                    { model: model.GradePermission, where: {permission_value: 1 }, include: [model.Grade, model.Permission] },
                    { model: model.ProfilePermission, include: [model.Permission] }
                  ]
                },
                model.Prefix
              ]
            ...
            console.dir(JSON.parse(JSON.stringify(clubs)), {depth: null});
          });
        ...
        //     console.log(findUser.getUserProfile());
        //     done();
        //   });
      });
    });
